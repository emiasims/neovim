...
local function safe_insert(t, pos, len, v)
...
local wrap = function(f, cb2tf_opts, cb_pos)
  return co.cb_to_tf(schedule_cb(f, cb_pos), cb2tf_opts)
end
...
M.timer_start = function(timer, timeout)
  ...
  return co.cb_to_tf(timer_start_cb)(timer, timeout)
end
...
M.shutdown = function(stream)
  local shutdown_cb = function(cb, stream_)
    ...
    local uv_shutdown, err, name = vim.uv.shutdown(stream_, cb)
  ...
  return co.cb_to_tf(shutdown_cb, {
...
M.write = function(stream, data)
  local write_cb = function(cb, stream_, data_)
    ...
    local handle, err, err_name = vim.uv.write(stream_, data_, cb)
